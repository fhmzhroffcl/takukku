...
extension DynamicNotch {
    ...
    func _expand(on screen: NSScreen = NSScreen.screens[0], skipHide: Bool) async {
        ...
        if needsNewWindow {
            ...
            if skipHide {
                withAnimation(effectiveConversionAnimation) { self.state = .expanded }
            } else {
            Task { @MainActor in
                if !skipHide {
        ...
        try? await Task.sleep(for: .seconds(skipHide ? 0.32 : 0.4))
    }
    ...
    func _compact(on screen: NSScreen = NSScreen.screens[0], skipHide: Bool) async {
        ...
        if needsNewWindow {
            ...
            if skipHide {
                withAnimation(effectiveConversionAnimation) { self.state = .compact }
            } else {
            Task { @MainActor in
                if !skipHide {
                    withAnimation(effectiveClosingAnimation) {
...
private extension DynamicNotch {
    ...
    func initializeWindow(screen: NSScreen, orderFront: Bool = true) {
        ...
        // Keep the non-activating panel close to the visible notch so it does not
        // intercept clicks across half of the user's display.
        let size = NSSize(
            width: min(680, screen.frame.width),
            height: min(360, screen.frame.height)
        )
        let origin = NSPoint(
